...
When working with single-cell transcriptomics data (or data with a very high number of conditions), clustering of cells (conditions) will typically be part of data preprocessing. Thus, to know which genes are characteristic for a given cluster, APL provides a possibility to upload a cluster annotation file instead of providing the IDs manually. The uploaded file should consists of two columns - the first one with the condition ID and the second one with the cluster ID. Importantly, the condition IDs in this file can be different from IDs in the input expression matrix, however they should be listed in the same order (the first row of cluster annotation file corresponds to the first condition from input expression matrix, the second - to the second, etc.). For instance, when processing single-cell data using the R package `Seurat`, such file can be easily obtained using the function `Idents`:
...
APL allows for enrichment analysis of the condition-specific genes for Gene Ontology terms by using the R package `topGO`, which facilitates such type of analyses.

To conduct gene enrichment analysis in APL first make sure that in the field `"Which conditions?"` the correct condition IDs are chosen or that the cluster annotation file is uploaded and the correct cluster IDs is selected. Next, in the field `"Annotation file"` in the side panel choose the correct gene ontology file (mouse or human) or provide the name of one of the Bioconductor AnnotationData Packages for another organism (look here: http://bioconductor.org/packages/3.10/data/annotation/ for the list of existing annotation packages). When using a new annotation package, please make sure that it is installed on your machine before launching APL.

After choosing the correct parameters the enrichment test using the Kolmogorov-Smirnov test will be conducted. The results in the tabular form will be displayed in the tab `"GSEA plot"`. The generated table will contain top ten biological processes enriched among top 1000 condition-specific genes obtained from the Association Plot (or if the total number of genes used for generating the Association Plot was lower than 1000 - 25% of number of genes used for this analysis). 

Another way of understanding the gene enrichment results is to investigate the distribution of the enriched GO terms over the entire GO graph. In APL, we use the property of `topGO` to generate such a subgraph and it is displayed below the table with the top 10 enriched GO categories. In the displayed subgragh the significant nodes are presented in form of rectangles, whereas color of the node represents the relative significance of the GO term: dark red is related to the highest significance and light yellow - to the lowest. Additionally, the generated GO subgraph is automatically saved as a pdf file in the working directory under the name `"GSEA_classic_10_def.pdf"`.
