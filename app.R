	...
	observeEvent(input$do, {
		...
		observeEvent(input$run_gsea, {		  
			...
			if(input$radio == 1){
				...
				chosen_annot_file <- "org.Hs.eg.db"	
			}else{ 
				if(input$radio == 2){
					output$gsea_errors <- renderPrint({
						paste0("Analysis conducted using mouse annotation file.")
					})			
				chosen_annot_file <- "org.Mm.eg.db"	
				}else{
					if(input$radio == 3){
						chosen_annot_file <- paste(input$annotIn)	
					}
				}
			}
			...
			output$gsea_errors <- renderPrint({		
				...
				paste0("The package ", chosen_annot_file, " is either not installed or is not available. Please install it or choose another annotation.")
			})							
			...
			if(require(chosen_annot_file, character.only = TRUE, quietly = T)){
				...
				tmp_ranking=tmp_ranking[order(tmp_ranking[,4]),c(4,1)]
				colnames(tmp_ranking)=c("Ranking","Gene_name")
				# select the number of top ranked genes for the enrichment analysis
				nr_genes_gsea = 1000
				...
				allGO2genes <- annFUN.org(whichOnto="BP", feasibleGenes=NULL, mapping= paste(chosen_annot_file), ID="symbol")
